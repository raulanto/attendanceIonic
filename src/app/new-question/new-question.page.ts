...
export class NewQuestionPage implements OnInit {
  questionUrl: string = "http://attendancedb1.test/questions"
     ...
     /*} else {
        const response = await axios({
          ...
          if (response?.status == 200) {
            this.alertGuardado(response.data.que_title, 'El alumno con matricula ' + response.data.alu_matricula + ' ha sido actualizado');
          }
        }).catch((error) => {
          if (error?.response?.status == 422) {
